...
jQuery(document).ajaxComplete(function (event, xhr, settings) {
    var ts = $.tablesorter, sorting = false, searching = false;
    $('table')
        .ready(function (e) { })
        .tablesorter({
        theme: 'blue',
    ...
    function showPopup(packageName, packageVersion, targetHost, managerHost) {
        var Actions = ['install', 'update', 'delete'];
    ...
    function generateLink(packageName, packageVersion, targetHost, managerHost, packageAction) {
        ...
        $('#modal-contents > ' + item + ' > form').submit(function (event) {
            ...
            jQuery.ajax({
                ...
                success: function (data) {
                    ...
                    jQuery("p.item-6").html("<p>" + Action + "の送信に成功しました。</p>");
                },
                error: function (xhr, ajaxOptions, thrownError) {
                    jQuery("p.item-5").html("<p>" + xhr.status + "<br>" + xhr.responseText + "<br>" + thrownError + "</p>");
                    jQuery("p.item-6").html("<p>" + Action + "の送信に失敗です。</p>");
                }
            });
            event.preventDefault();
        });
