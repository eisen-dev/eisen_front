...
<html lang="ja">
<head>
<title><?php echo _('Recipe list'); ?></title>
<?php
    ...
    <div class="contentswrapper menu-set">
        <main class="contents">
            <div class="content-header">
                <!-- page title -->
                <h2 class="title content-header-title"><?php echo _('Recipe list'); ?></h2>
            ...
            <form action="includes/recipe_checkbox.php" method="post">
                            ...
                            if (is_array($data[$i])) {
                                foreach ($data as $i => $array) {
                                    foreach ($array as $x => $task) {
                                        ...
                                        $table .= '<td class="host">' . $task->host . '</td>';
                                        ...
                                        $table .= '<td class="package">' . $task->package . '</td>';
                                        $table .= '<td class="file">' . $task->file . '</td></tr>';
                                        echo($table);
                                    }
                                }
                            }
...
<div class="modal" id="task_list-setting">
    <div class="modal-wrapper">
        <div class="modal-window">
            <form action="includes/recipe_registration.php" method="post">
                <div class="modal-header">
                    <i class="fa fa-times modal-close" data-modal="close"></i>
                    <span class="modal-title"><?php echo _('recipe settings'); ?></span>
                </div>
    ...
    function showPopup(task_id, managerHost) {
        $.ajax({
            ...
            .done(function (response) {
                ...
                $('.modal-header > .modal-title').text('recipe id: ' + response.return[0]);
